...
function bundleFiles(version) {
  ...
  if (!exe) {
    throw new Error(`installer Reel_${version}_x64-setup.exe not found in ${dir}`);
  }
  if (!sig) {
...
function publishToReleases(version, { dir, exe, sig }) {
  ...
  if (commits.trim() === "[]") {
    runOk(
      `${ghAuth()} api -X PUT /repos/${RELEASE_REPO}/contents/README.md ` +
        ...
        `-f content="${Buffer.from("Reel release artifacts and update manifest\n").toString("base64")}"`,
    );
  }
  ...
  run(
    `${ghAuth()} release create ${tag} --repo ${RELEASE_REPO} --title "Reel v${version}" ` +
